...
def main():
    ...
    ask_question = st.chat_input("Enter your prompt: ")

    
    if ask_question:

        if uploaded_file and ask_question:
            upload_pdf(uploaded_file)
            documents = load_pdf(pdfs_directory + uploaded_file.name)
            text_chunks = create_chunks(documents)
            faiss_db = create_vector_store(FAISS_DB_PATH, text_chunks, ollama_model_name)

            retrieved_docs=retrieve_docs(faiss_db, ask_question)
            response=answer_query(documents=retrieved_docs, model=llm_model, query=ask_question)

            st.chat_message("user").write(ask_question)
            st.chat_message("AI").write(response)
            st.session_state.messages.append({'role':'user', 'content': ask_question})
